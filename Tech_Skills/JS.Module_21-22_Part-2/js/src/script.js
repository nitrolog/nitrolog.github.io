...
$(function () {
  ...
  let questionList = localStorage.getItem('questionStr');

  questionList = JSON.parse(questionList);


              /**    Создаем шаблон для проверки ответов     **/
  let correctAnswer = ['stringify()', 'JSON', '<greeting>Hello, world!</greeting>'];
  let checkCorrectAnswer = [];



  let $content = _.template( $('#template').html() );
  let $body = $('body');
  let $modal;
  let $overlay;
  let $closeX;
  ...
  function showModal(e) {
    ...
    let $userAnswerCollect = $("input:radio:checked").parent('label');
